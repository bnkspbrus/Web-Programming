<template>
  <div id="app">
    ...
    <Footer :count-of-posts="Object.keys(posts).length" :count-of-users="Object.keys(users).length"/>
  </div>
</template>
...
export default {
  ...
  beforeCreate() {
    ...
    this.$root.$on("onRegister", (login, name, password) => {
      ...
      if (!(/^[a-z]+$/.test(login))) {
        this.$root.$emit("onRegisterValidationError", "Login must contain only small letters")
        return
      }
      ...
      if (password.length < 1 || password.length > 16) {
        this.$root.$emit("onRegisterValidationError", "Password length must be between 3 and 16")
        return
      }
      const users = Object.values(this.users).filter(u => u.login === login)
      if (users.length !== 0) {
      ...
      this.$root.$set(this.users, id, {id, login, name, admin: false})
