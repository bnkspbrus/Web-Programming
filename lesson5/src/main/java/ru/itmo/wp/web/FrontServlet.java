...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request,
        ...
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            ...
            for (Method method1 : pageClass.getDeclaredMethods()) {
                if (method1.getName().equals(route.getAction())) {
                    method = method1;
                    break;
                }
            }
        ...
        try {
            ...
            throw new ServletException(
                    "Can't invoke action method [pageClass=" + pageClass + ", method=" + method + "]");
        ...
        if (lang != null && !lang.equals("en")) {
            String nameWithLang = pageClass.getSimpleName() + "_" + lang + ".ftlh";
            try {
                template = newTemplate(nameWithLang);
        ...
        try {
            ...
            if (sourceConfiguration == null) {
                throw new ServletException("Can't render template [pageClass=" + pageClass + ", action=" + method + "]",
                        e);
            }
        }
    }
    ...
    private static class Route {
        ...
        private static Route newRoute(HttpServletRequest request) {
            ...
            List<String> classNameParts = Arrays.stream(uri.split("/")).filter(part -> !part.isEmpty()).collect(
                    Collectors.toList());
