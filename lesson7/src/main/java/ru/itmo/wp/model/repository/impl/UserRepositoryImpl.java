...
public class UserRepositoryImpl extends BasicRepositoryImpl<User> implements UserRepository {
    ...
    private static final String NAME = User.class.getSimpleName();
    ...
    public List<User> findAll() {
        ...
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("SELECT * FROM User ORDER BY id DESC")) {
                try (ResultSet resultSet = statement.executeQuery()) {
                    User user;
                    while ((user = toDomain(statement.getMetaData(), resultSet)) != null) {
                        users.add(user);
                    }
                }
            }
    ...
    protected User toDomain(ResultSetMetaData metaData, ResultSet resultSet) throws SQLException {
        if (!resultSet.next()) {
            return null;
        }
    ...
    @Override
    protected void setSaveStatementParams(PreparedStatement statement, User domain) throws SQLException {
    }
